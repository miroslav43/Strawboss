...
type ModalView = 'form' | 'pick_farm' | 'create_farm';

interface Props {
  visible: boolean;
  onSave: (data: { name: string; farmId: string | null; municipality: string; notes: string }) => void;
  onClose: () => void;
  isSaving: boolean;
}
...
export function CreateParcelModal({ visible, onSave, onClose, isSaving }: Props) {
  ...
  function resetAll() {
    setView('form');
    setName(''); setFarmId(null); setFarmName(''); setMunicipality(''); setNotes(''); setError('');
    setNewFarmName(''); setNewFarmPhone(''); setNewFarmEntityType('');
    setNewFarmCui(''); setNewFarmApia(''); setNewFarmAddress(''); setCreateFarmError('');
  }
  ...
  function handleSave() {
    ...
    setError('');
    onSave({ name: name.trim(), farmId, municipality: municipality.trim(), notes: notes.trim() });
    resetAll();
  }
  ...
  const handleCreateFarm = useCallback(async () => {
    if (!newFarmName.trim()) { setCreateFarmError('Numele fermei este obligatoriu.'); return; }
    if (!newFarmPhone.trim()) { setCreateFarmError('Numărul de telefon este obligatoriu.'); return; }
    setCreateFarmError('');
    ...
    try {
      ...
      setNewFarmName(''); setNewFarmPhone(''); setNewFarmEntityType('');
      setNewFarmCui(''); setNewFarmApia(''); setNewFarmAddress('');
      setView('form');
    } catch {
  ...
  }, [newFarmName, newFarmPhone, newFarmEntityType, newFarmCui, newFarmApia, newFarmAddress, queryClient]);
        ...
        <View style={[styles.container, { paddingTop: insets.top + 12, paddingBottom: insets.bottom + 8 }]}>
          ...
          {view === 'form' && (
            <>
              ...
              >
                {/* Name */}
                <Text style={styles.label}>Nume câmp <Text style={styles.required}>*</Text></Text>
                <TextInput
                  style={styles.input}
                  value={name}
          ...
          {view === 'pick_farm' && (
            <>
              <View style={styles.titleRow}>
                <TouchableOpacity style={styles.backBtn} onPress={() => setView('form')} hitSlop={8}>
                  ...
                  farms.map((farm) => (
                    ...
                    >
                      <View style={{ flex: 1 }}>
                        ...
                        {farm.address ? <Text style={styles.farmItemSub}>{farm.address}</Text> : null}
          ...
          {view === 'create_farm' && (
            <>
              <View style={styles.titleRow}>
                <TouchableOpacity style={styles.backBtn} onPress={() => setView('pick_farm')} hitSlop={8}>
                  <MaterialCommunityIcons name="arrow-left" size={22} color="#0A5C36" />
                  <Text style={styles.backBtnText}>Înapoi</Text>
                </TouchableOpacity>
              ...
              <ScrollView
                ...
                showsVerticalScrollIndicator={false}
              >
                <Text style={styles.label}>Nume fermă <Text style={styles.required}>*</Text></Text>
                <TextInput
                  style={styles.input}
                  value={newFarmName}
                ...
                <View style={styles.segmentRow}>
                  <TouchableOpacity
                    style={[styles.segmentBtn, newFarmEntityType === FarmEntityType.persoana_juridica && styles.segmentBtnActive]}
                    onPress={() => setNewFarmEntityType(
                      newFarmEntityType === FarmEntityType.persoana_juridica ? '' : FarmEntityType.persoana_juridica
                    )}
                  >
                    <Text style={[styles.segmentText, newFarmEntityType === FarmEntityType.persoana_juridica && styles.segmentTextActive]}>
                      Persoană juridică
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[styles.segmentBtn, newFarmEntityType === FarmEntityType.persoana_fizica && styles.segmentBtnActive]}
                    onPress={() => setNewFarmEntityType(
                      newFarmEntityType === FarmEntityType.persoana_fizica ? '' : FarmEntityType.persoana_fizica
                    )}
                  >
                    <Text style={[styles.segmentText, newFarmEntityType === FarmEntityType.persoana_fizica && styles.segmentTextActive]}>
                      Persoană fizică
                    </Text>
                  </TouchableOpacity>
                </View>
              ...
              <View style={styles.actions}>
                <TouchableOpacity style={styles.cancelBtn} onPress={() => setView('pick_farm')} disabled={creatingFarm}>
...
const styles = StyleSheet.create({
  ...
  saveBtn: { flex: 2, paddingVertical: 14, borderRadius: 10, backgroundColor: '#0A5C36', alignItems: 'center' },
  saveBtnDisabled: { backgroundColor: '#9CA3AF' },
  saveBtnText: { fontSize: 15, fontWeight: '700', color: '#fff' },
  farmItem: {
    flexDirection: 'row', alignItems: 'center',
    paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: '#F3F4F6', gap: 8,
  },
