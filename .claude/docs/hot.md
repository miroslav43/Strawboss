...
- **CMR scan for auxiliary loads (Jul 2026):** an external driver on an auxiliary load photographs the paper CMR at drop-off — mobile scans it (camera fallback, ML Kit pre-fetch), builds a PDF on-device, and uploads via the new backend `CmrScansModule` (`attachScan()`: per-request advisory-lock retire+insert, PDF magic-byte sniffing); admin can also override-upload from the Trip Requests page (`CmrUploadModal`, size caps mirror backend `AVIZ_MAX_BYTES`/`CMR_SCAN_MAX_BYTES`). New `document_type` enum value `cmr_scan` (migration **00083**, distinct from the backend-generated `cmr`) and `TripRequest.hasCmrScan` flag. ([[backend]], [[mobile]], [[admin-web]], [[database]], [[packages-types]], [[packages-api]])
- **Mobile reliability & traffic-diet follow-ups (Jul 2026):** FCM data-wake handler (`remote-notification-task.ts`) plus a 90s check-in temporal gate (`CHECKIN_GATE_MS`) to skip redundant network round-trips; GPS background batching (`POST /location/report/batch`, 30-item chunks, 60s flush gate); widened polling cadence across 6 hooks with a `focusManager` fix that now actually halts background polling; heartbeat interval 30s→~60s (55s self-dedup gate); `SyncManager` parcels `sync_version` fix (was hardcoded 0); `auth.ts` single-flight refresh guard; app version bumped vc37→vc41. ([[mobile]])
