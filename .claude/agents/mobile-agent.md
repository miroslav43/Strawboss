...
**Fix-source tagging (vc56 / 1.0.52, Aug 2026)**: `coordsToReport()` now takes a `source: 'task' | 'checkin'` param (default `'task'`) stamped onto every `LocationReportDto`. `getBestEffortPosition()` — the alarm-driven fallback used when an OEM ROM freezes the app between native-alarm ticks (`presence-checkin-task.ts`'s 60 s tick) — tags both its last-known and Balanced-accuracy branches `'checkin'`, and now returns `null` (posts nothing) when the fix admits more than `CHECKIN_MAX_USEFUL_ACCURACY_M = 2_000` m of error. **Any new caller of `getBestEffortPosition`/`coordsToReport` must preserve the `source` tag** — the backend uses it to keep presence-only fixes off drawn tracks and distance reports (migration `00097`, see `.claude/docs/backend.md` "GPS Noise Filtering"); silently defaulting a genuinely-background fix to `'task'` would reintroduce the county-wide "spider web" bug this field exists to fix.
...
### i18n / Locale (`src/lib/i18n.tsx`) — trilingual since Aug 2026

`SUPPORTED_LOCALES` in `@strawboss/types` is `['ro', 'en', 'hu']`; catalogs are `src/i18n/{ro,en,hu}.ts`. No in-app language picker — an admin sets `users.locale` from the web app; the phone reads it off the Zustand auth store via `normalizeLocale()` (now delegates to the SSOT, `0e8647a`). `t()`/`tStatic()` fall back `locale → en → ro` (English before Romanian — a missing key must surface internationally-readable, not local). `dateLocaleFor(locale)` returns the BCP-47 tag for every `toLocaleDateString`/`Intl.*` call; before Aug 2026 mobile had zero locale branching there (everything was hardcoded `'ro-RO'`). See [[mobile]] "i18n / Locale" for the full writeup.
...
17. **Adding a locale**: add the code to `SUPPORTED_LOCALES` in `packages/types/src/locale.ts`, then create `src/i18n/<code>.ts` here (mirroring `ro.ts`'s shape). Nothing else to edit — the `Record<Locale, …>` catalog map in `i18n.tsx` and every `CatalogShape<T>`-checked sibling catalog (`en.ts`/`hu.ts`) won't compile until the new file exists and has full key parity with `ro.ts`. Never leave a catalog leaf as an array — `CatalogShape` maps that to `never` by design (`771de60`).
