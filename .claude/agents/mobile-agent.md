...
**Check-in (`runDeviceCheckin()`)**: public endpoint — `fleetApiClient` sends **no `Authorization` header**. Collects device UUID/token, app version code, hardware info (`getDeviceHardwareInfo()`), FCM push token, `isDeviceOwner` flag, active-trip flag, and any batched OTA progress reports. POSTs to `POST /api/v1/fleet/checkin`.
...
Two new `@ReactMethod` entries in `DeviceOwnerModule.kt`:
...
Generated Kotlin class `InstallResultReceiver` (in `DEVICE_OWNER_MODULE_KT`) logs `STATUS_SUCCESS` / `STATUS_PENDING_USER_ACTION` (warning, should not occur on device owner) / failure — best-effort only, since the JS promise resolves before the broadcast and the process may be dead.
