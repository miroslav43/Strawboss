...
import type { Alert } from '@strawboss/types';
...
export function useAlerts(client: ApiClient, filters?: Record<string, unknown>) {
  return useQuery({
    ...
    queryFn: () => {
      ...
      return client.get<Alert[]>(`/api/v1/alerts${params}`);
    },
  });
}
...
export function useUnacknowledgedAlerts(client: ApiClient) {
  return useQuery({
    ...
    queryFn: () => client.get<Alert[]>('/api/v1/alerts/unacknowledged'),
  });
}
...
export function useAcknowledgeAlert(client: ApiClient) {
  ...
  return useMutation({
    mutationFn: ({ id, resolutionNotes }: { id: string; resolutionNotes?: string }) =>
      client.patch<Alert>(`/api/v1/alerts/${id}/acknowledge`, { resolutionNotes }),
    onSuccess: () => {
