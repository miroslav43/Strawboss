...
const spacingTailwind = Object.fromEntries(Object.entries(spacing).map(([k, v]) => [k, `${v}px`]));
...
export const strawbossPreset = {
  theme: {
    extend: {
      ...
      lineHeight: Object.fromEntries(Object.entries(lineHeights).map(([k, v]) => [k, String(v)])),
    },
  },
} as const;
